...
-- NOTE: your old config used `source = home-monitors.conf` to pull in monitor
-- lines from a separate file. The Lua equivalent is `require`, but it expects
...
hl.bind(mainMod .. " + M", hl.dsp.window.fullscreen({ mode = 1 })) -- toggle window fullscreen aka monocle
...
-- Switch workspaces with mainMod + [0-9] (AZERTY row: & é " ' ( - è _ ç à)
-- NOTE: your original .conf had no key bound for switching TO workspace 10
-- (the `bind = $mainMod, , workspace, 10` line has an empty key field) —
-- that looks like a leftover/typo, since moving a window to workspace 10
-- IS bound to SUPER+SHIFT+à below. Preserved as-is; add a key here if you
-- meant to bind it.
...
-- (workspace 10 switch bind intentionally left unbound, see note above)
